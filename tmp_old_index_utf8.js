...
app.post('/api/calls/outbound', async (req, res) => {
  if (!vapi) {
    return res.status(503).json({ error: 'VAPI ╨╜╨╡ ╨╜╨░╤Б╤В╤А╨╛╨╡╨╜. ╨г╨║╨░╨╢╨╕╤В╨╡ VAPI_API_KEY.' });
  }
  const phoneNumberId = process.env.VAPI_PHONE_NUMBER_ID;
  const assistantId = req.body?.assistantId || process.env.VAPI_ASSISTANT_ID;
  if (!phoneNumberId || !assistantId) {
    return res.status(400).json({
      error: '╨Э╨╡ ╤Г╨║╨░╨╖╨░╨╜╤Л VAPI_PHONE_NUMBER_ID ╨╕ VAPI_ASSISTANT_ID.',
    });
  }
  const phoneNumber = req.body?.phoneNumber?.trim();
  if (!phoneNumber) {
    return res.status(400).json({ error: '╨г╨║╨░╨╢╨╕╤В╨╡ ╨╜╨╛╨╝╨╡╤А ╤В╨╡╨╗╨╡╤Д╨╛╨╜╨░' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: err.message || '╨Э╨╡ ╤Г╨┤╨░╨╗╨╛╤Б╤М ╨╖╨░╨┐╤Г╤Б╤В╨╕╤В╤М ╨╖╨▓╨╛╨╜╨╛╨║' });
  }
});
